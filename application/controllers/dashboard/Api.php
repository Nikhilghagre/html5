...
class Api extends CI_Controller {
    ...
    private function api_key()
    {
        ...
        if ($query->num_rows() > 0) {
			...
			$api_key = base64_decode(strrev(convert_uudecode($query->row()->api_key)));
            return $api_key;
        }
    }
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if ($result == TRUE) {
                        if ($this->input->post('user_email') != null) {
                            ...
                            $verification_url = base_url()."dashboard/Auth/email_verification/".base64_encode(strrev(base64_encode($this->input->post('user_email'))))."/".base64_encode(strrev(base64_encode($user_email_verification_code)));
                            $to = $this->input->post('user_email');
                            $cc = false; //To send a copy of email
            ...
            if ($access_key_received == $this->api_key()) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if ($query->num_rows() > 0)
                    {
                        ...
                        }elseif ($update_coin_type == "rewardedAd") {
                            $current_time = time(); // Time Now
                            $expired_at = $this->input->post('expiration_time');
                            $update_coin_time = $query->row()->update_coin_time; // Time that click on ads before
                            if ($update_coin_time + ($expired_at) < $current_time) {
    ...
    public function get_pc_info(){
        if (isset($_GET['api_key']))
        {
            ...
            if ($access_key_received != $this->api_key()) {
				...
				$purchase_code = config_item('purchase_code');
